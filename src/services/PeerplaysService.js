...
class PeerplaysService {
   ...
   */
  getTransferFee() {
    return this.callBlockchainDbApi('get_required_fees', [[[0]], '1.3.0']).then((result) => {
  ...
  authAccount(fullAccount, password) {
    ...
    const activeKeyAuths = fullAccount.getIn(['account', 'active', 'key_auths']).toJS();
    const ownerKeyAuths = fullAccount.getIn(['account', 'owner', 'key_auths']).toJS();
    
    const keys = Login.generateKeys(accountName, password,
      ['active'],
  ...
  /**
   * Create and broadcast a transaction with loggedin user's ppy account name and password.
   *
   * @param {string} opName - Operation name.
   * @param {string} opJson - JSON of operation.
   * @param {string} peerplaysAccountName - Username for peerplays account.
   * @param {string} peerplaysPassword - Password for peerplays account.
   * @returns {string} Result of transaction.
   * @memberof PeerplaysService
   */
